...
def main():
    ...
    if missing:
        ...
        # 2026-09-11 补这三行。此前这里只把缺口列出来，**却完全不影响退出码** ——
        # 屏幕上先是一串「还没有测试的：…」，紧接着一个绿勾，
        # 读的人（不管是人还是 AI）会得出「绿了就是没事」。
        # 这正是 F21：**让绿勾去替一件它根本没判的事背书**。
        # 修法不是让它变红 —— 覆盖率真的不归这个检查器判，
        # 半路让它红，实现方在施工中途就会天天见红，见多了就会忽略它。
        # 修法是**把「这一维归谁判」当场说出来**。
        print("   ⚠️ 覆盖到没到，**不归这个检查器判**，下面那个绿勾也不为它背书。")
        print("      判覆盖的是 M0/T0 与 `_check_delivery.py`（读 CI 的 surefire XML）。")
        print("      本检查器只判三条红线：没动别人的 / 没删没禁 / 断言没变少。")
    print()
