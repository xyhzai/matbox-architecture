...
用户 2026-09-11 拍板：**真库测试只在 CI 跑，本地不起库**。原话「要我拍板什么呢，我不明白，你直接拍就可以啊」—— 授权我定，我定的是这个。理由：CI 本来就是权威判据（真起 postgres:16、真跑 Flyway、真回查建表），选它现在就能继续，用户不用动手；代价是每验一次要推上去等几分钟。嫌慢再改成本地起容器，那时只改这一处。
"""
...
def main():
    ...
    # 数据库这一条是**可选**的，依据是用户的拍板（见文件头 DECISION）。
    # 它曾经是硬性的 —— 改成可选不是放宽标准，是把「本地跑不了真库测试」
    # 从「缺东西」重新归类成「已定的工作方式」。两者的区别在于：
    # 前者要人去补，后者要人知道**本地绿不等于交付绿**。
    url = os.environ.get("MATBOX_DB_URL", "")
    ok, msg = db_reachable(url)
    if ok:
        print("  ✅ 数据库 %s —— 本地可以跑真库测试" % msg)
    else:
        print("  ⚪ 数据库 %s" % msg)
        print("       —— 按用户 2026-09-11 的拍板，**这不算缺**：")
        print("          本地只跑 `mvn -B -DskipTests verify`（保证编得过），")
        print("          真库测试推上分支交给 CI（它真起 postgres:16）。")
        print("          **本地绿 ≠ 交付绿** —— 交付以 CI 的红绿为准。")
    ...
    print("  · postgresql：已由用户拍板，不用你操心 —— 真库测试交给 CI。")
    return 1
