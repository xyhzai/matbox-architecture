...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp CRED-F004`：
...
**目标**：AI 员工调用外部系统时，密钥不经过 AI 员工本身，由中间层代理注入
...
### Backend/API/Service｜Applicable · CRED-F004 Owner

本功能的服务端实现

### Data/DB/Migration｜Applicable · CRED-F004 Owner
...
### Provider/External｜Applicable · CRED-F004 Owner
...
| 编号 | 要验证什么 |
|---|---|
| CRED-T541 | 对 AI 员工的输出做扫描（复用 F-DQ-005 Betterleaks），0 泄露。这是不可降级 P0 |
| CRED-T001 | AI员工输出内容扫描测试（复用F-DQ-005 Betterleaks），验证CRED-F004代理注入后，Prompt/日志/生成代码中0次出现真实密钥字符串。 |

### 4.1 测试方法名必须带 TestID —— 这是硬要求

测试方法名必须写成 `credT541_说明()` 这种形状，编号就是上面那张表里的编号。

```java
@Test
void credT541_describeWhatThisVerifies() {
    // 断言这一条验收标准真的成立
}
```
