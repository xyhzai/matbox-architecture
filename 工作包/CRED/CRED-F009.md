# CRED-F009 · 匿名访客会话身份（Anonymous Visitor Session）
...
| Owner | Platform |
| 分支前缀 | `wp/` |
| 认领的验收标准 | **CRED-T591**、**CRED-T592**、**CRED-T593**、**CRED-T005**、**CRED-T006** |
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp CRED-F009`：
...
**目标**：独立站未登录消费者的会话级临时身份，可升级关联到正式Contact身份，2026-08-30从调研清单#9并入
...
### Backend/API/Service｜Applicable · CRED-F009 Owner

本功能的服务端实现

### Data/DB/Migration｜Applicable · CRED-F009 Owner
...
### Provider/External｜Applicable · CRED-F009 Owner
...
| 编号 | 要验证什么 |
|---|---|
| CRED-T591 | 匿名会话默认权限只覆盖只读allowlist（如浏览public产品目录、跟该租户公开AI员工对话） |
| CRED-T592 | 身份升级后原会话上下文正确关联到新Contact身份且不可被其他访客冒领 |
| CRED-T593 | 会话过期或长时间不活跃后自动失效 |
| CRED-T005 | 匿名访客会话尝试访问allowlist之外的资源，验证TENANT-F006判断为DENY（P0冻结规则第5条）。 |
| CRED-T006 | 访客会话升级关联测试——留资后session上下文正确转移到新Contact身份，且原会话不可被其他访客冒领。 |

### 4.1 测试方法名必须带 TestID —— 这是硬要求

测试方法名必须写成 `credT591_说明()` 这种形状，编号就是上面那张表里的编号。

```java
@Test
void credT591_describeWhatThisVerifies() {
    // 断言这一条验收标准真的成立
}
```
