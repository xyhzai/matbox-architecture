...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp CRED-F003`：
...
**目标**：每个 AI Agent 的独立身份 + 短期限权 Token 签发
...
### Backend/API/Service｜Applicable · CRED-F003 Owner

本功能的服务端实现

### Data/DB/Migration｜Applicable · CRED-F003 Owner
...
### Provider/External｜Applicable · CRED-F003 Owner
...
| 编号 | 要验证什么 |
|---|---|
| CRED-T531 | Token 默认有效期不超过单次任务预期时长 |
| CRED-T532 | 同一 AI 员工不同任务的 Token 互不复用 |

### 4.1 测试方法名必须带 TestID —— 这是硬要求

测试方法名必须写成 `credT531_说明()` 这种形状，编号就是上面那张表里的编号。

```java
@Test
void credT531_describeWhatThisVerifies() {
    // 断言这一条验收标准真的成立
}
```
