...
| Owner | Platform |
| 分支前缀 | `wp/` |
| 认领的验收标准 | **CRED-T581**、**CRED-T582**、**CRED-T003** |
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp CRED-F008`：
...
**目标**：AI员工的持久身份（跟着EmployeeInstance走），跟CRED-F003的任务级临时Grant是两层不同的东西，2026-08-30从AI员工底层包核对后补充
...
### Backend/API/Service｜Applicable · CRED-F008 Owner

本功能的服务端实现

### Data/DB/Migration｜Applicable · CRED-F008 Owner
...
### Provider/External｜Applicable · CRED-F008 Owner
...
| 编号 | 要验证什么 |
|---|---|
| CRED-T581 | AgentIdentity被Revoke/Retire时，名下所有未过期的AgentCredentialGrant必须在同一事务或紧随的补偿动作里同步失效 |
| CRED-T582 | delegated_by字段记录委派链条，委派产生的Identity权限不得超过源Identity授权范围（对应F-ACTION-001 ACTION-F005的委派权限检查） |
| CRED-T003 | AgentIdentity被Revoke时，验证其名下所有ACTIVE状态的AgentCredentialGrant在同一事务/紧随补偿动作内全部转为REVOKED（对应CRED-F008验收标准，级联失效测试）。 |

### 4.1 测试方法名必须带 TestID —— 这是硬要求

测试方法名必须写成 `credT581_说明()` 这种形状，编号就是上面那张表里的编号。

```java
@Test
void credT581_describeWhatThisVerifies() {
    // 断言这一条验收标准真的成立
}
```
