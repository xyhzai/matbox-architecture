...
| Owner | Platform |
| 分支前缀 | `wp/` |
| 认领的验收标准 | **CRED-T511**、**CRED-T512**、**CRED-T007**、**CRED-T008**、**CRED-T009** |
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp CRED-F001`：
...
**目标**：连接自建 Infisical 实例，项目/环境结构设计
...
### Backend/API/Service｜Applicable · CRED-F001 Owner

本功能的服务端实现

### Data/DB/Migration｜Applicable · CRED-F001 Owner
...
### Provider/External｜Applicable · CRED-F001 Owner
...
| 编号 | 要验证什么 |
|---|---|
| CRED-T511 | 任何模块要读密钥，必须经过这一层 |
| CRED-T512 | 直接硬编码或裸调用视为违规，对应 F-DQ-007 架构守卫的检查项之一 |
| CRED-T007 | Infisical实例不可用场景下的fail-closed测试——模拟`PROVIDER_UNAVAILABLE`，验证Grant签发/密钥读取均返回拒绝而非静默放行。 |
| CRED-T008 | 在真实自托管测试环境、无Enterprise License Key的条件下，验证Dynamic Secrets创建接口与`secretPath`级条件权限配置的真实可用性，形成Stage 10前的书面证据。 |
| CRED-T009 | 全新自托管Infisical安装 → 创建项目 → 创建密钥的完整链路回归测试，确认不命中"Missing Project KMS Key"已知问题。 |

### 4.1 测试方法名必须带 TestID —— 这是硬要求

测试方法名必须写成 `credT511_说明()` 这种形状，编号就是上面那张表里的编号。

```java
@Test
void credT511_describeWhatThisVerifies() {
    // 断言这一条验收标准真的成立
}
```
