# CRED-F007 · 企业第三方集成凭证（Integration Credential Vault）
...
| Owner | Platform |
| 分支前缀 | `wp/` |
| 认领的验收标准 | **CRED-T571**、**CRED-T572**、**CRED-T573**、**CRED-T574**、**CRED-T575**、**CRED-T576** |
...
cd matbox
git checkout -b wp/cred-f007
cd backend/modules/credential
# 本地不起库（用户 2026-09-11 拍板），只保证编得过
mvn -B -DskipTests verify
git push -u origin HEAD
# 真正的验证在 GitHub Actions：它真起 postgres:16 跑完整 mvn -B verify
```
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp CRED-F007`：
...
**目标**：企业连第三方服务（非AI员工/非人类账号）的OAuth凭证，2026-08-30从多端平台文档并入
...
### Backend/API/Service｜Applicable · CRED-F007 Owner

本功能的服务端实现

### Data/DB/Migration｜Applicable · CRED-F007 Owner
...
### Provider/External｜Applicable · CRED-F007 Owner
...
| 编号 | 要验证什么 |
|---|---|
| CRED-T571 | Backend/API/Service：CredentialVaultService/OAuth回调/运行时注入；模型/业务service不得直接读secret；secret redaction + 过期/撤销token拒绝 |
| CRED-T572 | Data/DB/Migration：只存集成元数据+加密ref/版本，不存明文secret；DB dump无可用明文credential |
| CRED-T573 | Async/Runtime：refresh/rotation过期提醒；AI任务不得自己刷新token；重复refresh幂等、race受控 |
| CRED-T574 | Provider：OAuth/KMS/Secret Manager适配器；业务代码不绑定provider token格式；provider可替换测试 |
| CRED-T575 | Infra/Config：KMS密钥/轮换/IAM；Git/env里不留长期明文secret；最小权限+轮换演练 |
| CRED-T576 | Tests/Observability：secret泄露扫描/撤销/轮换/审计测试；日志/错误/证据0 secret泄露 |

### 4.1 测试方法名必须带 TestID —— 这是硬要求

测试方法名必须写成 `credT571_说明()` 这种形状，编号就是上面那张表里的编号。

```java
@Test
void credT571_describeWhatThisVerifies() {
    // 断言这一条验收标准真的成立
}
```
