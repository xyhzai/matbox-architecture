...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp CRED-F002`：
...
### Backend/API/Service｜Applicable · CRED-F002 Owner

本功能的服务端实现

### Data/DB/Migration｜Applicable · CRED-F002 Owner
...
### Provider/External｜Applicable · CRED-F002 Owner
...
| 编号 | 要验证什么 |
|---|---|
| CRED-T521 | 两人账号互相独立，可单独查日志区分谁做的操作 |
| CRED-T004 | 人类账号和AI身份共用同一凭据的防护测试——构造尝试用同一credential同时登录两种主体类型的用例，验证被拒绝（P0冻结规则第3条）。 |

### 4.1 测试方法名必须带 TestID —— 这是硬要求

测试方法名必须写成 `credT521_说明()` 这种形状，编号就是上面那张表里的编号。

```java
@Test
void credT521_describeWhatThisVerifies() {
    // 断言这一条验收标准真的成立
}
```
