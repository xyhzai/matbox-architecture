...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp CRED-F005`：
...
### Backend/API/Service｜Applicable · CRED-F005 Owner

本功能的服务端实现

### Data/DB/Migration｜Applicable · CRED-F005 Owner
...
### Provider/External｜Applicable · CRED-F005 Owner
...
| 编号 | 要验证什么 |
|---|---|
| CRED-T551 | 轮换记录可追溯，轮换失败有告警，不能"改了但不知道" |
| CRED-T002 | 同一AgentIdentity在Grant过期后，再次调用`GET /cred/agent-grants/{grantId}`必须返回EXPIRED状态且TENANT-F003鉴权判断为DENY。 |

### 4.1 测试方法名必须带 TestID —— 这是硬要求

测试方法名必须写成 `credT551_说明()` 这种形状，编号就是上面那张表里的编号。

```java
@Test
void credT551_describeWhatThisVerifies() {
    // 断言这一条验收标准真的成立
}
```
