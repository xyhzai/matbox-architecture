# LOC-F003 · Localization Catalog & Key Governance

> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F003 自己那一份。
...
**目标**：实现 LOC-F003 —— Localization Catalog & Key Governance。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F003`：
...
### Backend/API/Service｜Applicable · LOC-F003 Owner
...
**AC-LOC-032**（归属来源：人裁（附理由）、关键词推导、端点表）
- **Given** 全球多语言模块已按本包契约部署，租户与用户上下文已就绪
...
```java
@Test
@Tag("AC-LOC-004")                    // 方便单独跑：mvn test -Dgroups=AC-LOC-004
void locT004_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
...
| 方法 | 路径 | 幂等键 |
|---|---|---|
| GET | `/loc/catalog/{locale}` | — |
| POST | `/loc/catalog/candidates` | catalog_version |
