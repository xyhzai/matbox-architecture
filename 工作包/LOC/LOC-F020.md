...
> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F020 自己那一份。
...
| Stage 9 状态 | READY_FOR_STAGE10_BINDING / RELEASE_GATE_OPEN |
| Implementer | opencode |
| ParallelReadiness | UNRESOLVED_UNTIL_REAL_REPO_BINDING |
| ⚠️ 阻塞 | **被 G-LOC-MATBOX-002 阻塞：依赖的 Messaging Owner 模块在本图中不存在** |

> ⛔ **这个功能现在不要动。** 被 G-LOC-MATBOX-002 阻塞：依赖的 Messaging Owner 模块在本图中不存在
...
**目标**：实现 LOC-F020 —— Realtime Multilingual Chat & Mobile AI Command Bridge。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F020`：
...
### Backend/API/Service｜Applicable · LOC-F020 Owner
...
```java
@Test
@Tag("AC-LOC-033")                    // 方便单独跑：mvn test -Dgroups=AC-LOC-033
void locT033_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
