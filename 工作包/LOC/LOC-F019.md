# LOC-F019 · AI Language Quality Gate / Agent Language Tooling

> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F019 自己那一份。
...
**目标**：实现 LOC-F019 —— AI Language Quality Gate / Agent Language Tooling。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F019`：
...
### Backend/API/Service｜Applicable · LOC-F019 Owner
...
```java
@Test
@Tag("AC-LOC-027")                    // 方便单独跑：mvn test -Dgroups=AC-LOC-027
void locT027_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
...
| 方法 | 路径 | 幂等键 |
|---|---|---|
| POST | `/loc/quality/validate` | content_or_message_ref |
