# LOC-F002 · User/Tenant Language & Region Preferences

> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F002 自己那一份。
...
**目标**：实现 LOC-F002 —— User/Tenant Language & Region Preferences。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F002`：
...
### Backend/API/Service｜Applicable · LOC-F002 Owner
...
```java
@Test
@Tag("AC-LOC-001")                    // 方便单独跑：mvn test -Dgroups=AC-LOC-001
void locT001_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
...
| 方法 | 路径 | 幂等键 |
|---|---|---|
| PUT | `/loc/preferences/me` | user_id |
| GET | `/loc/preferences/me` | — |
| PUT | `/loc/tenants/{tenantId}/defaults` | tenant_id |
