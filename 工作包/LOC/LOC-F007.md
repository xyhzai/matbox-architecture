# LOC-F007 · Site/Page Locale Integration

> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F007 自己那一份。
...
**目标**：实现 LOC-F007 —— Site/Page Locale Integration。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F007`：
...
### Backend/API/Service｜Applicable · LOC-F007 Owner
...
**AC-LOC-000**（占位 · 源文档缺口）
- **Given** 源文档 DEV.docx §9 里没有任何一条 AC 点到本功能
- **When** 有人要按本包开工
- **Then** 必须先补出本功能的验收标准；**没有验收标准就无法判定做完没做完**，不许硬塞别人的条目充数。

> 🔴 **本功能是源文档的真实缺口**，不是我忘了填。由 `_build_loc_workpackages.py` 如实标注。
