# LOC-F008 · Business Content Translation Store

> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F008 自己那一份。
...
| ParallelReadiness | UNRESOLVED_UNTIL_REAL_REPO_BINDING |
...
**目标**：实现 LOC-F008 —— Business Content Translation Store。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F008`：
...
### Backend/API/Service｜Applicable · LOC-F008 Owner

### Data/DB/Migration｜Applicable · 见 5.2
...
```java
@Test
@Tag("AC-LOC-005")                    // 方便单独跑：mvn test -Dgroups=AC-LOC-005
void locT005_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
...
| 方法 | 路径 | 幂等键 |
|---|---|---|
| GET | `/loc/translations` | — |
| POST | `/loc/translations` | sha256(tenant+object_type+object_id+field_key+source_hash+target_locale) |
| GET | `/loc/translations/{translationId}` | — |
