# LOC-F017 · Localization Regression & Acceptance Gate

> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F017 自己那一份。
...
**目标**：实现 LOC-F017 —— Localization Regression & Acceptance Gate。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F017`：
...
### Backend/API/Service｜Applicable · LOC-F017 Owner
...
```java
@Test
@Tag("AC-LOC-032")                    // 方便单独跑：mvn test -Dgroups=AC-LOC-032
void locT032_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
