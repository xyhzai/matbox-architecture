# LOC-F012 · Glossary / Terminology / Placeholder Integrity

> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F012 自己那一份。
...
**目标**：实现 LOC-F012 —— Glossary / Terminology / Placeholder Integrity。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F012`：
...
### Backend/API/Service｜Applicable · LOC-F012 Owner
...
```java
@Test
@Tag("AC-LOC-014")                    // 方便单独跑：mvn test -Dgroups=AC-LOC-014
void locT014_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
...
| 方法 | 路径 | 幂等键 |
|---|---|---|
| POST | `/loc/translations/{translationId}/review` | translation_id+review_seq |
| GET | `/loc/terminology/{profileRef}` | — |
