# LOC-F014 · AI Employee / Conversation Locale Bridge

> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F014 自己那一份。
...
**目标**：实现 LOC-F014 —— AI Employee / Conversation Locale Bridge。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F014`：
...
### Backend/API/Service｜Applicable · LOC-F014 Owner
...
```java
@Test
@Tag("AC-LOC-008")                    // 方便单独跑：mvn test -Dgroups=AC-LOC-008
void locT008_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
...
| 方法 | 路径 | 幂等键 |
|---|---|---|
| POST | `/loc/ai/session/language` | conversation_ref |
