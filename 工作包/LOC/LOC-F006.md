# LOC-F006 · Tolgee Community TMS Bridge

> **这是一份 WorkPackage 交接文件，不是总账。** 只装 LOC-F006 自己那一份。
...
**目标**：实现 LOC-F006 —— Tolgee Community TMS Bridge。
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp LOC-F006`：
...
### Backend/API/Service｜Applicable · LOC-F006 Owner

### Data/DB/Migration｜N/A

### Async/Runtime｜Applicable · 共享 Reliable Runtime

### Provider/External｜Applicable · 翻译 Provider / TMS
...
```java
@Test
@Tag("AC-LOC-012")                    // 方便单独跑：mvn test -Dgroups=AC-LOC-012
void locT012_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
...
| 方法 | 路径 | 幂等键 |
|---|---|---|
| POST | `/loc/tms/sync` | tms_sync_ref |
