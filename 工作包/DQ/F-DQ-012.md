...
> **这是一份 WorkPackage 交接文件，不是总账。** 只装 F-DQ-012 自己那一份。13 个功能的完整总账在 [Matbox_代码持续质检与安全自检_开发文档_FINAL.md](../../Matbox_代码持续质检与安全自检_开发文档_FINAL.md)，本文件不重复它，也不要求你读它。
...
|---|---|
| FeatureID | `F-DQ-012` |
| 名称 | AI 代码审查补充证据（AI Code Review Supplemental Evidence Adapter） |
| 归属 | Platform Core Quality / DevCodeQuality |
| DocID | MATBOX-DEVQUALITY-GATE-20260819-V1.1-RC |
| Stage 9 状态 | OPTIONAL_B_PROVIDER · Stage10 可按成本/区域启用 |
| 阻塞 Gate | NONE for P0 core；启用时绑定 provider policy |
| 建议 WorkPackage 批次 | `WP-DQ-09`（同批：F012）—— AI Review optional adapter。 |
...
**Owner**：Feature Owner: Source Review Owner；Implementer: UNASSIGNED_STAGE10；Source Reviewer: independent human/AI policy；Acceptance Owner: QA Owner
...
判据是 `docs/_check_delivery.py --reports <那个目录> --wp F-DQ-012`：
...
**依赖**：Provider Router/F-CRED/F-COST/F-OBS；F-DQ-002

## 2 · 你能改哪里，不能碰哪里

**允许写入**：

- `backend/modules/dev-quality/src/main/java/com/matbox/devquality/adapters/ai-review/**`
...
```java
@Test
@Tag("DQ-T015")                       // 方便单独跑：mvn test -Dgroups=DQ-T015
void dqT015_describeWhatThisCriterionVerifies() {   // ← 后半段换成「这条在验什么」，英文小驼峰
...
2. 一条 TestID 可以有多个测试方法（`dqT015_a`、`dqT015_b`…），但**不能一条都没有**
...
- **.docx §16 第6条**：发往外部 Provider 的代码范围必须受一份显式白名单约束，由负责人决定哪些仓库/路径允许发出，不得因追求判断准确率而无限制外发。
...
**出处**：派工身份/Scope/依赖/Owner 来自 `.docx` 第9节表26；七类施工面来自第10节；验收标准来自第13节（归属为推导）；可写路径来自 `dq_protected_scope.yml`；架构禁令来自 `Matbox_架构设计原则.md`。